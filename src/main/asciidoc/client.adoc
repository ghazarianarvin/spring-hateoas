...
[source, java, indent=0, tabsize=2]
----
include::{base-dir}/src/test/java/org/springframework/hateoas/config/WebTestClientConfigurerTest.java[tag=web-test-client]
----
<1> Register your configuration class that uses `@EnableHypermediaSupport` to enable HAL support.
<2> Use `WebTestClientConfigurer` to apply hypermedia support.
...
class WebClientBasedTests {
    ...
    @Test
    void exampleTest(@Autowired WebTestClient client, @Autowired WebTestClientConfigurer configurer) { // <2>
        client = configurer.registerHypermediaTypes(client); // <3>
...
----
<1> This is Spring Boot's test annotation that will configure a `WebTestClient` for this test class.
<2> Autowire Spring Boot's `WebTestClient` into `client` and Spring HATEOAS's configurer as method parameters.
<3> Use `WebTestClientConfigurer` to register support for hypermedia.
