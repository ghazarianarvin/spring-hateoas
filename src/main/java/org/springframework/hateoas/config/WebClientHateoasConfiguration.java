...
class WebClientHateoasConfiguration {
	...
	@Bean
	@Lazy
	WebTestClientConfigurer webTestClientConfigurer(ObjectProvider<ObjectMapper> mapper,
			List<HypermediaMappingInformation> hypermediaTypes) {
		return new WebTestClientConfigurer(mapper.getIfAvailable(ObjectMapper::new), hypermediaTypes);
	}
